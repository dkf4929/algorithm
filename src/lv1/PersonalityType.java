...
public class PersonalityType {
    public String solution(String[] survey, int[] choices) {
        ...
        // 선택한 결과에 대한 점수
        int[] scoreArray = {3, 2, 1, 0, 1, 2, 3};

        // 각 유형에 대한 점수를 저장할 map
        ...
        for (int i = 0; i < survey.length; i++) {
            ...
            int score = scoreArray[choices[i] - 1];

            if (choices[i] == 0) continue;
            else if (choices[i] > 4) {
                Integer typeScore = map.getOrDefault(type[1], 0);
                map.replace(type[1], typeScore + score);
            } else {
                Integer typeScore = map.getOrDefault(type[0], 0);
                map.replace(type[0], typeScore + score);
            }
        }
        ...
        for (int i = 0; i < keys.size(); i=i+2) {
            ...
            String second = keys.get(i + 1);

            Integer firstScore = map.get(first);
            Integer secondScore = map.get(second);

            if (firstScore > secondScore) answer.append(first);
            else if (secondScore > firstScore) answer.append(second);
