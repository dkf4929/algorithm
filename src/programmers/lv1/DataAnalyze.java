    ...
    public int[][] solution(int[][] data, String ext, int val_ext, String sort_by) {
        ...
        return list.stream()
                .filter(intArray -> {
                    // 기준이 date일 경우
                    if (findIdx == 1) {
                        String dateString = String.valueOf(intArray[findIdx]);
                        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyyMMdd");
                        LocalDate date = LocalDate.parse(dateString, formatter);

                        if (date.isBefore(LocalDate.parse(String.valueOf(val_ext), formatter))) return true;
                        else return false;
                    } else {
                        if (intArray[findIdx] < val_ext) return true;
                        else return false;
                    }
                })
                .sorted(Comparator.comparing(intArray -> {
                    return intArray[map.get(sort_by)];
                }))
                .toArray(int[][]::new);
    }
}
