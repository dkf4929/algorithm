...
public class SameColor {
    public int solution(String[][] board, int h, int w) {
        ...
        if (w != 0) leftColor = board[h][w-1];
        if (w < boardWidth - 1) rightColor = board[h][w+1];
        if (h != 0) upColor = board[h-1][w];
        if (h < boardHeight - 1) underColor = board[h+1][w];

        if (findColor.equals(leftColor)) answer++;
        if (findColor.equals(rightColor)) answer++;
        if (findColor.equals(upColor)) answer++;
        if (findColor.equals(underColor)) answer++;
